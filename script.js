...
// ==========================================
// 1. ИНИЦИАЛИЗАЦИЯ И СЕТЬ (ОБНОВЛЕННАЯ)
// ==========================================
...
peer.on('error', (err) => {
    ...
    if (err.type === 'peer-unavailable') {
        alert("Игрок с таким ID не найден! Проверьте ID.");
    } else {
        alert("Сетевая ошибка: " + err.type + "\nПроверьте консоль (F12).");
    }
});
...
peer.on('connection', (connection) => {
    conn = connection;
    isHost = true;
    statusEl.innerText = "Друг подключается...";
    setupConnection();
});

// Если МЫ подключаемся к другу
document.getElementById('connect-btn').addEventListener('click', () => {
    const enemyId = document.getElementById('enemy-id').value.trim(); // .trim() убирает случайные пробелы
    
    if(!enemyId) {
        alert("Сначала введи ID друга!");
        return;
    }
    
    statusEl.innerText = "Установка связи...";
    statusEl.style.color = "#ffdd59";
...
function setupConnection() {
    conn.on('open', () => {
        ...
        initBoards();
        randomizeFleet(); // Авторасстановка кораблей
    });
...
function checkStart() {
    if (amIReady && isEnemyReady) {
        ...
        isMyTurn = !isHost; // Тот кто подключается (не хост), ходит первым
        updateTurnUI();
    }
}
...
// ==========================================
// 4. МЕХАНИКИ (ОБРАБОТКА КЛИКА ПО ВРАГУ)
// ==========================================
...
function handleNetworkData(data) {
    ...
    // ВРАГ СТРЕЛЯЕТ В НАС
        ...
        // Если враг попал в мину, он пропускает ход (мы ходим снова)
        if (result.status !== 'mine') isMyTurn = true; 
        updateTurnUI();
        checkLose();
    }
    // ОТВЕТ ОТ ВРАГА НА НАШ ВЫСТРЕЛ
    ...
    // ВРАГ ИСПОЛЬЗУЕТ РАДАР
        ...
        for(let dy = -1; dy <= 1; dy++){
            for(let dx = -1; dx <= 1; dx++){
                ...
                if(nx >= 0 && nx < 10 && ny >= 0 && ny < 10 && myGrid[ny][nx] === 1) {
                    found = true;
                }
            }
        }
    ...
    // ВРАГ ИСПОЛЬЗУЕТ АВИАУДАР
        ...
        let results = targets.map(t => {
            if(t.x >= 0 && t.x < 10 && t.y >= 0 && t.y < 10) {
                return {x: t.x, y: t.y, res: processIncomingAttack(t.x, t.y)};
            }
            return null;
        }).filter(r => r !== null);
...
// Вспомогательная логика: обработка урона на нашем поле
function processIncomingAttack(x, y) {
    let cell = myBoardEl.children[y * 10 + x];
    
    if (myGrid[y][x] === 1) { // Попадание в корабль
        myGrid[y][x] = -1; // Подбит
    ...
    else if (myGrid[y][x] === 2) { // Попадание в мину
...
// Визуализация результатов наших выстрелов по врагу
...
function checkLose() {
    // Проверяем, осталась ли хоть одна неподбитая палуба
    let hasAlive = myShips.some(ship => ship.some(p => !p.hit));
    if (!hasAlive) {
        conn.send({ type: 'WIN' });
        setTimeout(() => { 
            alert("💀 ТЫ ПРОИГРАЛ! Твой флот на дне."); 
            location.reload(); 
        }, 500);
    }
}
...
document.getElementById('skill-radar').onclick = () => { 
    currentAction = 'radar'; 
    toggleCancelBtn(true); 
};

document.getElementById('skill-airstrike').onclick = () => { 
    currentAction = 'airstrike'; 
    toggleCancelBtn(true); 
};
...
function toggleCancelBtn(show) {
    ...
    enemyBoardEl.style.borderColor = show ? '#ffd32a' : '#4bcffa'; // Подсветка поля желтым при выборе скилла
}
