...
let conn = null; // Будущее соединение с другом
...
function setupConnection() {
    conn.on('open', () => {
        statusEl.innerText = "Статус: Игра началась! Расставляй корабли (пока отключено)";
        document.getElementById('connection-panel').style.display = 'none';
        gameZone.style.display = 'block';
    });

    // Обработка входящих данных (выстрелов, скиллов)
    conn.on('data', (data) => {
        if (data.type === 'SHOOT') {
            console.log(`Противник выстрелил в ${data.x}, ${data.y}`);
            // TODO: Проверить попадание и отправить ответ
            const cell = document.querySelector(`#my-board .cell[data-x="${data.x}"][data-y="${data.y}"]`);
            cell.classList.add('miss'); // Пока всё мимо
        }
    });
}
...
function handleShoot(x, y) {
    if (!conn) return alert("Сначала подключитесь к другу!");
